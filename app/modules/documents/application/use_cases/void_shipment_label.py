...
class VoidShipmentLabelCommand:
    tenant_id: UUID
    shipment_label_id: UUID


class VoidShipmentLabelUseCase:
    def __init__(
        self,
    ...
    async def execute(
        self,
        command: VoidShipmentLabelCommand,
    ) -> ShipmentLabel:
        async with self._unit_of_work:
            ...
            if shipment_label.status not in {
                LabelStatus.PENDING,
                LabelStatus.GENERATED,
            }:
                raise InvalidShipmentLabelStateTransitionError

            shipment_label.status = LabelStatus.VOIDED

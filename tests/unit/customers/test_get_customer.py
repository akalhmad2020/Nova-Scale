from datetime import UTC, datetime
from uuid import uuid4

import pytest

from app.modules.customers.application.exceptions import (
    CustomerNotFoundError,
)
from app.modules.customers.application.use_cases.get_customer import (
    GetCustomer,
    GetCustomerQuery,
)
from app.modules.customers.domain.enums import CustomerStatus
from app.modules.customers.infrastructure.models.customer import Customer
from tests.unit.customers.fakes import FakeUnitOfWork


async def test_get_customer_returns_customer() -> None:
    uow = FakeUnitOfWork()

    tenant_id = uuid4()

    customer = Customer(
        tenant_id=tenant_id,
        name="Acme Trading",
        code="ACME-001",
        status=CustomerStatus.ACTIVE,
    )

    uow.customers.add(customer)

    use_case = GetCustomer(uow)

    result = await use_case.execute(
        GetCustomerQuery(
            tenant_id=tenant_id,
            customer_id=customer.id,
        )
    )

    assert result is customer


async def test_get_customer_rejects_unknown_customer() -> None:
    uow = FakeUnitOfWork()

    use_case = GetCustomer(uow)

    with pytest.raises(CustomerNotFoundError):
        await use_case.execute(
            GetCustomerQuery(
                tenant_id=uuid4(),
                customer_id=uuid4(),
            )
        )

    assert uow.rolled_back is True


async def test_get_customer_rejects_customer_from_other_tenant() -> None:
    uow = FakeUnitOfWork()

    customer = Customer(
        tenant_id=uuid4(),
        name="Other Tenant Customer",
        code="OTHER-001",
        status=CustomerStatus.ACTIVE,
    )

    uow.customers.add(customer)

    use_case = GetCustomer(uow)

    with pytest.raises(CustomerNotFoundError):
        await use_case.execute(
            GetCustomerQuery(
                tenant_id=uuid4(),
                customer_id=customer.id,
            )
        )

    assert uow.rolled_back is True


async def test_get_customer_rejects_soft_deleted_customer() -> None:
    uow = FakeUnitOfWork()

    tenant_id = uuid4()

    customer = Customer(
        tenant_id=tenant_id,
        name="Deleted Customer",
        code="DELETED-001",
        status=CustomerStatus.ACTIVE,
    )
    customer.deleted_at = datetime.now(UTC)

    uow.customers.add(customer)

    use_case = GetCustomer(uow)

    with pytest.raises(CustomerNotFoundError):
        await use_case.execute(
            GetCustomerQuery(
                tenant_id=tenant_id,
                customer_id=customer.id,
            )
        )

    assert uow.rolled_back is True

    ...
    *,
    ...
    try:
        async with session_factory() as session:
            ...
            role = Role(
                name=role_name,
                description="Shipment update integration role",
            )
...
async def create_foreign_resources(
    *,
    ...
    try:
        async with session_factory() as session:
            tenant = Tenant(
                name="Foreign Shipment Update Tenant",
                slug=tenant_slug,
                status=TenantStatus.ACTIVE,
            )
...
async def test_update_shipment_endpoint_updates_shipment() -> None:
    ...
    email = f"shipment-update-{unique}@example.com"
    ...
    try:
        ...
        assert body["tracking_number"] == "NEW-001"
...
@pytest.mark.integration
async def test_update_shipment_endpoint_requires_permission() -> None:
    unique = uuid4()

    email = f"shipment-update-denied-{unique}@example.com"
    password = "very-secure-shipment-password"
...
@pytest.mark.integration
async def test_update_shipment_endpoint_rejects_unknown_shipment() -> None:
    unique = uuid4()

    email = f"shipment-update-missing-{unique}@example.com"
    password = "very-secure-shipment-password"
...
@pytest.mark.integration
async def test_update_shipment_endpoint_rejects_duplicate_tracking_number() -> None:
    unique = uuid4()

    email = f"shipment-update-duplicate-{unique}@example.com"
    password = "very-secure-shipment-password"
    ...
    try:
        ...
        with TestClient(app) as client:
            response = client.patch(
                f"/api/v1/tenants/{tenant.id}/shipments/{second.id}",
                headers={
                    "Authorization": f"Bearer {access_token}",
                },
                json=shipment_payload(
                    ...
                    origin_location_id=origin.id,
                    destination_location_id=destination.id,
                    tracking_number=first.tracking_number.lower(),
                ),
            )
...
async def test_update_shipment_endpoint_enforces_resource_tenant_isolation(
    foreign_resource: str,
    expected_detail: str,
) -> None:
    unique = uuid4()

    email = f"shipment-update-isolation-{unique}@example.com"
    password = "very-secure-shipment-password"

    tenant_slug = f"shipment-update-isolation-{unique}"
    ...
    try:
        shipment = await create_shipment(
            ...
            origin_location_id=origin.id,
            destination_location_id=destination.id,
            tracking_number="SHIP-ISOLATION",
        )

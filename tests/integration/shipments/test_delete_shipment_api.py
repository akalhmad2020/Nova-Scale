    ...
    try:
        async with session_factory() as session:
            ...
            permission = await session.scalar(
                select(Permission).where(Permission.code == Permissions.SHIPMENT_DELETE)
            )
...
async def test_delete_shipment_endpoint_soft_deletes_shipment() -> None:
    ...
    tenant, customer, origin, destination = await create_delete_context(
        email=email,
        password=password,
...
async def test_delete_shipment_endpoint_rejects_unknown_shipment() -> None:
    ...
    tenant, _, _, _ = await create_delete_context(
        email=email,
        password=password,
        tenant_slug=tenant_slug,
        role_name=role_name,

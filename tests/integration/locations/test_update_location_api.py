    ...
    *,
    ...
    try:
        async with session_factory() as session:
            ...
            role = Role(
                name=role_name,
                description="Location update integration role",
            )

            permission = await session.scalar(
                select(Permission).where(Permission.code == Permissions.LOCATION_UPDATE)
            )
...
async def test_update_location_endpoint_updates_location() -> None:
    unique = uuid4()

    email = f"location-update-{unique}@example.com"
    password = "very-secure-location-password"
    tenant_slug = f"location-update-{unique}"
    role_name = f"location-update-role-{unique}"
    ...
    try:
        ...
        assert body["name"] == "New Warehouse"
        assert body["code"] == "WH-002"
        assert body["type"] == "pickup"
        assert body["country_code"] == "JO"
        assert body["state"] == "Amman"
        assert body["city"] == "Amman"
        assert body["postal_code"] == "11118"
        assert body["address_line1"] == "New Address"
        assert body["address_line2"] == "Building 10"
        assert body["contact_name"] == "Operations"
        assert body["email"] == "ops@example.com"
        assert body["phone"] == "+962790000000"
        assert body["latitude"] == "31.953900"
        assert body["longitude"] == "35.910600"
        assert body["notes"] == "Updated location"
...
@pytest.mark.integration
async def test_update_location_endpoint_requires_permission() -> None:
    unique = uuid4()

    email = f"location-update-denied-{unique}@example.com"
    password = "very-secure-location-password"
    tenant_slug = f"location-update-denied-{unique}"
    role_name = f"location-update-denied-role-{unique}"
...
@pytest.mark.integration
async def test_update_location_endpoint_rejects_unknown_location() -> None:
    unique = uuid4()

    email = f"location-update-missing-{unique}@example.com"
    password = "very-secure-location-password"
    tenant_slug = f"location-update-missing-{unique}"
    role_name = f"location-update-missing-role-{unique}"
...
@pytest.mark.integration
async def test_update_location_endpoint_rejects_duplicate_code() -> None:
    unique = uuid4()

    email = f"location-update-duplicate-{unique}@example.com"
    password = "very-secure-location-password"
    tenant_slug = f"location-update-duplicate-{unique}"
    role_name = f"location-update-duplicate-role-{unique}"
    ...
    try:
        ...
        with TestClient(app) as client:
            response = client.patch(
                f"/api/v1/tenants/{tenant.id}/locations/{second.id}",
                headers={
                    "Authorization": f"Bearer {access_token}",
                },
                json={
        ...
        assert response.json() == {"detail": "Location code already exists"}

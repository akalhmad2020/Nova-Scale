    ...
    *,
    ...
    try:
        async with session_factory() as session:
            ...
            tenant = Tenant(
                name="Pricing Read Integration Tenant",
                slug=tenant_slug,
            ...
            role = Role(
                name=role_name,
                description="Pricing read integration role",
            )

            permission = await session.scalar(
                select(Permission).where(Permission.code == Permissions.PRICING_RULE_READ)
            )
...
async def create_foreign_context(
    *,
    ...
    try:
        async with session_factory() as session:
            tenant = Tenant(
                name="Foreign Pricing Read Tenant",
                slug=tenant_slug,
                status=TenantStatus.ACTIVE,
            )
...
async def test_list_pricing_rules_endpoint_returns_tenant_rules() -> None:
    unique = uuid4()

    email = f"pricing-read-list-{unique}@example.com"
    password = "very-secure-pricing-password"
    tenant_slug = f"pricing-read-list-tenant-{unique}"
    role_name = f"pricing-read-list-role-{unique}"
    ...
    try:
        first = await create_pricing_rule(
            tenant_id=tenant.id,
            name="Standard Rule",
        )

        second = await create_pricing_rule(
            tenant_id=tenant.id,
            name="Express Rule",
...
@pytest.mark.integration
async def test_list_pricing_rules_endpoint_returns_empty_list() -> None:
    unique = uuid4()

    email = f"pricing-read-empty-{unique}@example.com"
    password = "very-secure-pricing-password"
    tenant_slug = f"pricing-read-empty-tenant-{unique}"
    role_name = f"pricing-read-empty-role-{unique}"
...
@pytest.mark.integration
async def test_list_pricing_rules_endpoint_requires_permission() -> None:
    unique = uuid4()

    email = f"pricing-read-denied-{unique}@example.com"
    password = "very-secure-pricing-password"
    tenant_slug = f"pricing-read-denied-tenant-{unique}"
    role_name = f"pricing-read-denied-role-{unique}"
...
@pytest.mark.integration
async def test_get_pricing_rule_endpoint_returns_rule() -> None:
    unique = uuid4()

    email = f"pricing-get-{unique}@example.com"
    password = "very-secure-pricing-password"
    tenant_slug = f"pricing-get-tenant-{unique}"
    role_name = f"pricing-get-role-{unique}"
...
@pytest.mark.integration
async def test_get_pricing_rule_endpoint_rejects_unknown_rule() -> None:
    unique = uuid4()

    email = f"pricing-get-missing-{unique}@example.com"
    password = "very-secure-pricing-password"
    tenant_slug = f"pricing-get-missing-tenant-{unique}"
    role_name = f"pricing-get-missing-role-{unique}"
...
@pytest.mark.integration
async def test_get_pricing_rule_endpoint_rejects_foreign_rule() -> None:
    unique = uuid4()

    email = f"pricing-get-foreign-{unique}@example.com"
    password = "very-secure-pricing-password"

    tenant_slug = f"pricing-get-tenant-{unique}"
    foreign_slug = f"pricing-get-foreign-tenant-{unique}"
    role_name = f"pricing-get-foreign-role-{unique}"
    ...
    _, foreign_rule = await create_foreign_context(
